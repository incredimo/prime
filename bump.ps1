...
# Check if at least one target was built successfully
$windowsTargetInstalled = Test-TargetInstalled "x86_64-pc-windows-msvc"
$linuxTargetInstalled = Test-TargetInstalled "x86_64-unknown-linux-gnu"

# If no targets are installed, warn the user but continue
if (-not ($windowsTargetInstalled -or $linuxTargetInstalled)) {
    Write-Output "⚠️ No cross-compilation targets installed. Please install at least one target to build binaries:"
    Write-Output "   For Windows: rustup target add x86_64-pc-windows-msvc"
    Write-Output "   For Linux: rustup target add x86_64-unknown-linux-gnu"
    Write-Output "Continuing without building binaries..."
}
